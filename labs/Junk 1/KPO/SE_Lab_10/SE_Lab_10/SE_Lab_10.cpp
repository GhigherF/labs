#include <algorithm>
#include<iostream>
#include <vector>

using namespace std;

int main()	
{
	setlocale(LC_ALL, "rus");
	vector<int> v{1,2,3,4,5,6,7,3,9,10 };
	int target1 = 3;
	int num_items1 = count(v.begin(), v.end(), target1);
	cout <<endl<<"Число:"<< target1 << "Кол-во:" << endl << num_items1 << endl << endl << endl;
	int num_items3 = count_if(v.begin(), v.end(), [](int i) {return i % 3 == 0; });
	cout << "Кол-во кратных:"<<num_items3 << endl;
	cout << endl<<"Захват:" << endl;
	for (auto i : v)[i]() {if (i % 3 == 0)cout << i << " "; }();
	cout << endl << "С параметрами" << endl;
	for (auto i : v)[](auto i) {if (i % 3 == 0)cout << i << " "; }(i);
	cout << endl << "без параметров" << endl;
	for (auto i : v)[i] {if (i % 3 == 0)cout << i << " "; }();
}

